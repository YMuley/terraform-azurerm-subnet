...
resource "azurerm_subnet" "subnet" {
 ...
 service_endpoints              = each.value.service_endpoints == "" ? "" : each.value.service_endpoints
 service_endpoint_policy_ids    = each.value.service_endpoint_policy_ids == "" ? "" : flatten([ for service_endpoint_policy in var.service_endpoint_policy_output : service_endpoint_policy.id  if contains(each.value.service_endpoint_policy_ids,service_endpoint_policy.name) == true ])
